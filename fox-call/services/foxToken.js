...
export function decodeFoxToken(token) {
  try {
    const t = token.trim();
    const idx = t.indexOf(':');
    ...
    const pt = xorBytes(ct, key);
    let text;
    try { text = utf8Decode(pt); } catch { return null; }

    const parts = text.split('|');
    if (parts.length !== 4) return null;
    const embUid = parts[0], serverUrl = parts[1], nonce = parts[2], tag = parts[3];
...
export function encodeFoxToken(userId, serverUrl) {
