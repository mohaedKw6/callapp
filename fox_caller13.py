...
# -*- coding: utf-8 -*-
"""
Fox Caller v13.0 - temp-mail.io Edition
========================================
Email provider: temp-mail.io API v3 (from Instant Mail APK)
...
# temp-mail.io API v3 (extracted from Instant Mail APK)
TEMPMAIL_IO_API = "https://api.internal.temp-mail.io"
TEMPMAIL_DOMAIN   = ""  # Custom domain (set via --domain)
# Fallback: tempmail.lol API
TEMPMAIL_LOL_API = "https://api.tempmail.lol"
TEMPMAIL_LOL_KEY  = "tempmail.20260604.vmu8bbm2vo5rurre32cnojhp84h7um7kuswa5uqwzq1pewhv"


# ═══════════════════════════════════════════════════════════════
# ─── temp-mail.io API v3 (from Instant Mail APK) ──────────────
# ═══════════════════════════════════════════════════════════════
_tempmail_lock = threading.Lock()

def tempmail_create_inbox(prefix=None, domain=None, debug=False):
    """
    Create a temp email inbox.
    Uses temp-mail.io API v3 (from Instant Mail APK reverse engineering).
    Falls back to tempmail.lol API if that fails.
    
    temp-mail.io API v3:
      POST /api/v3/email/new
      Body: {} or {"domain":"..."} or {"name":"..."}
      Response 200: {"email":"...","token":"..."}
    
    tempmail.lol API (fallback):
      POST /v2/inbox/create
      Body: {"domain":"...","prefix":"..."}
      Response 201: {"address":"...","token":"..."}
    
    Returns (email_address, inbox_token, provider) or (None, None, None)
    """
    body = {}
    if domain:
        body["domain"] = domain
    ...
    # === Method 1: temp-mail.io API v3 (WORKS from Egypt!) ===
    try:
        r = requests.post(f"{TEMPMAIL_IO_API}/api/v3/email/new",
                          headers={"Content-Type": "application/json"},
                          json=body, timeout=20)
        if debug:
            print(f"    [DEBUG] temp-mail.io: HTTP {r.status_code} | {r.text[:200]}", flush=True)
        if r.status_code == 200:
            ...
            if email and token:
                if debug:
                    print(f"    [DEBUG] temp-mail.io: SUCCESS! {email}", flush=True)
                return email, token, 'temp-mail.io'
    except Exception as e:
        if debug:
            print(f"    [DEBUG] temp-mail.io: {type(e).__name__}: {e}", flush=True)
    
    # === Method 2: tempmail.lol API (fallback, may be blocked in Egypt) ===
    lol_body = {}
    if domain:
        lol_body["domain"] = domain
    if prefix:
        lol_body["prefix"] = prefix
    
    lol_headers = {"Content-Type": "application/json", "Authorization": f"Bearer {TEMPMAIL_LOL_KEY}"}
    try:
        r = requests.post(f"{TEMPMAIL_LOL_API}/v2/inbox/create",
                          headers=lol_headers, json=lol_body, timeout=20)
        if debug:
            print(f"    [DEBUG] tempmail.lol: HTTP {r.status_code} | {r.text[:200]}", flush=True)
        if r.status_code == 201:
            ...
            if addr and tok:
                if debug:
                    print(f"    [DEBUG] tempmail.lol: SUCCESS! {addr}", flush=True)
                return addr, tok, 'tempmail.lol'
    except Exception as e:
        if debug:
            print(f"    [DEBUG] tempmail.lol: {type(e).__name__}: {e}", flush=True)
    
    return None, None, None


def tempmail_get_emails(email_address, inbox_token, provider='temp-mail.io'):
    """
    Fetch emails for an inbox.
    Uses different API depending on provider.
    
    temp-mail.io: GET /api/v3/email/{email_address}/messages
    tempmail.lol:  GET /v2/inbox?token={inbox_token}
    """
    try:
        if provider == 'temp-mail.io':
            r = requests.get(f"{TEMPMAIL_IO_API}/api/v3/email/{email_address}/messages", timeout=20)
            if r.status_code == 200:
                return r.json()  # Returns list of email objects
        else:
            r = requests.get(f"{TEMPMAIL_LOL_API}/v2/inbox",
...
def tempmail_read_otp(email_address, inbox_token, timeout=OTP_TIMEOUT, sent_time=None, provider='temp-mail.io'):
    """
    Poll inbox for OTP from Telicall.
    ...
    while time.time() < deadline:
        ...
        emails = tempmail_get_emails(email_address, inbox_token, provider)
        ...
        for email_data in emails:
            ...
            # Extract OTP from body/html/subject
            body_text = email_data.get("body", "") or ""
            html_text = email_data.get("html", "") or ""
            subject = email_data.get("subject", "") or ""
            combined = body_text + " " + html_text + " " + subject
...
def test_tempmail_connection():
    """Test email API at startup. Returns (success, domain_used, read_ok, provider)."""
    for attempt in range(3):
        try:
            debug_mode = (attempt == 0)
            addr, token, provider = tempmail_create_inbox(domain=TEMPMAIL_DOMAIN or None, debug=debug_mode)
            if addr and token:
                domain = addr.split('@')[1] if '@' in addr else '?'
                emails = tempmail_get_emails(addr, token, provider)
                read_ok = isinstance(emails, list)
                return True, domain, read_ok, provider
            else:
    ...
    # Try raw connection tests
    for name, url in [("temp-mail.io", TEMPMAIL_IO_API), ("tempmail.lol", TEMPMAIL_LOL_API)]:
        try:
            r = requests.get(url, timeout=10)
            print(f"    Raw connection to {name}: HTTP {r.status_code}", flush=True)
        except Exception as e:
            print(f"    Cannot reach {name}: {type(e).__name__}: {e}" , flush=True)
...
def discover_domains(count=10):
    ...
    for i in range(count):
        addr, _, _ = tempmail_create_inbox(domain=TEMPMAIL_DOMAIN or None)
        if addr and '@' in addr:
            domain = addr.split('@')[1]
...
def _try_one_phone(phone, duration, mode, tid):
    ...
    email_addr, inbox_token, email_provider = tempmail_create_inbox(prefix=prefix, domain=domain, debug=False)
    ...
    # Each inbox is completely separate - no race conditions!
    otp = tempmail_read_otp(email_addr, inbox_token, timeout=OTP_TIMEOUT, sent_time=sent_time, provider=email_provider)
    if not otp:
...
# ═══════════════════════════════════════════════════════════════
def main():
    global _start_time, _phone_queue, TEMPMAIL_LOL_KEY, TEMPMAIL_DOMAIN
    
    parser = argparse.ArgumentParser(description="Fox Caller v13.0 - temp-mail.io Edition")
    ...
    parser.add_argument("--api-key", type=str, default=TEMPMAIL_LOL_KEY,
    ...
    # Apply overrides
    if args.api_key:
        TEMPMAIL_LOL_KEY = args.api_key
    if args.domain:
        TEMPMAIL_DOMAIN = args.domain
    
    print("\n" + "=" * 60, flush=True)
    print("  Fox Caller v13.0 - temp-mail.io Edition", flush=True)
    print("  Temp email via API - No Gmail, No IMAP!", flush=True)
    print("=" * 60, flush=True)
    
    # 1. Test tempmail.lol connection
    print(f"\n  Testing tempmail.lol API...", flush=True)
    tm_ok, tm_domain, tm_read, tm_provider = test_tempmail_connection()
    if tm_ok:
        print(f"  Email API: OK! Provider: {tm_provider} | Domain: {tm_domain} | Read: {'OK' if tm_read else 'FAIL'}", flush=True)
    else:
        print(f"  Email API: FAILED! Check internet connection", flush=True)
        sys.exit(1)
    ...
    if telicall_status == "ACCEPTED":
        ...
        if TEMPMAIL_DOMAIN:
            ...
            print(f"\n  WARNING: Default temp email domains may be BLOCKED by Telicall!", flush=True)
            print(f"  Solution: Use a custom domain that Telicall accepts:", flush=True)
            print(f"    python3 fox_caller13.py numbers.xlsx --domain yourdomain.com", flush=True)
            print(f"\n  Continuing anyway... (may get BLOCKED errors)", flush=True)
    
    # 3. Show config
    print(f"\n  API Key:    {TEMPMAIL_LOL_KEY[:20]}...", flush=True)
