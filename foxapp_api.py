...
# ═══════════════════════════════════════════════════════════════════════════════
#  Fox Caller API — upload accounts and make calls
# ═══════════════════════════════════════════════════════════════════════════════

@app.post("/api/fox-caller/upload-accounts")
def api_fox_caller_upload_accounts():
    """Upload account tokens from fox_caller1.py to the server.

    Auth: x-admin-key header

    Body: {
        "accounts": [
            {"email": "...", "x-client-device-id": "...", "x-token": "..."},
            ...
        ]
    }
    """
    ...
    for acc in accounts_data:
        ...
        token = acc.get("x-token") or acc.get("token", "")
        if email and device_id and token:
            cv.add_ready_token(email, device_id, token)
            # Also add to accounts list so start_call() can use them
            with cv._token_lock:
                existing_emails = {a.get("email") for a in cv.accounts if isinstance(a, dict)}
...
@app.post("/api/fox-caller/make-call")
def api_fox_caller_make_call():
    """Make a SIP call from the server using loaded accounts.
    ...
    The server will:
    1. Get an account from the ready tokens
    2. Start a call via Telicall API to get SIP credentials
    3. Make the SIP call from the server
    4. Stay in the call for `duration` seconds
    5. Return the result
    """
    admin_key = request.headers.get("x-admin-key", "")
    if not admin_key or not hmac_mod.compare_digest(admin_key, ADMIN_SECRET):
    ...
    try:
        duration = int(duration)
        duration = max(1, min(duration, 600))  # 1s to 10min
    ...
    call_thread = threading.Thread(target=_do_call, daemon=True)
    call_thread.start()

    # Wait for the call to complete (with generous timeout)
    # Call duration + 120s overhead for ringing, retries, etc.
    ...
    if not result_event.is_set():
        return jsonify({
            "status": "timeout",
            "message": f"Call did not complete within {timeout}s",
        }), 504

    result = call_result.get("result")

    if result and isinstance(result, tuple) and len(result) >= 2:
        success, call_from, rec_data = result[0], result[1], result[2] if len(result) > 2 else b''
        return jsonify({
            "status": "answered_ok" if success else "failed",
            "phone": phone,
            "from": call_from or "",
            "duration": duration if success else 0,
            "success": bool(success),
        })
    elif result is False or (isinstance(result, tuple) and result[0] is False):
        return jsonify({
            "status": "failed",
            "phone": phone,
            "success": False,
            "message": "Call failed - no answer, declined, or error",
        })
    else:
        return jsonify({
            "status": "error",
